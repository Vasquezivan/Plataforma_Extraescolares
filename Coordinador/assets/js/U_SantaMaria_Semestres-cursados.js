...
document.addEventListener('DOMContentLoaded', async function() {
  ...
  function inicializarSemestres() {
    ...
    tarjetasExistentes.forEach(tarjeta => {
      ...
      // En la vista de Coordinador no se muestran botones de editar/eliminar
  ...
  // Ocultar botón crear para vista de coordinador
  if (btnCrearSemestre) {
  ...
  if (btnCrearSemestre) {
    btnCrearSemestre.addEventListener('click', function() {
      if (modalSemestre) modalSemestre.style.display = 'block';
      const tituloModalEl = document.getElementById('tituloModal');
      if (tituloModalEl) tituloModalEl.textContent = 'Crear Nuevo Semestre';
      const semestreIdEl = document.getElementById('semestreId');
      if (semestreIdEl) semestreIdEl.value = '';
      semestreEnEdicion = null;
      if (formSemestre && typeof formSemestre.reset === 'function') formSemestre.reset();
    });
  }
  ...
  function agregarTarjetaSemestre(semestre, insertAtStart = true) {
    ...
    // Contenido de la tarjeta (solo visualización para Coordinador)
    tarjeta.innerHTML = `
  ...
  function abrirPanelAdministrador(semestre) {
    ...
    // Indicar la unidad académica para enrutar correctamente
    sessionStorage.setItem('unidad_academica', 'Unidad académica Tlahuitoltepec');

    // Redirigir al panel específico de la Unidad (vista coordinador)
    window.location.href = 'U_SantaMaria.html';
  }
  ...
  if (iconoPerfil) {
    ...
    try { iconoPerfil.removeEventListener('click', abrirPerfil); } catch (e) {}
    iconoPerfil.addEventListener('click', abrirPerfil);
    iconoPerfil.addEventListener('keydown', function(e) { if (e.key === 'Enter' || e.keyCode === 13) abrirPerfil(e); });

    document.addEventListener('click', function delegated(e) {
      try {
        if (e.target && e.target.closest && e.target.closest('#iconoPerfil')) abrirPerfil(e);
      } catch (err) {}
    });
  }
