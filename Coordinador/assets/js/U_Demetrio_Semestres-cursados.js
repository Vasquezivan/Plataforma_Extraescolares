...
document.addEventListener('DOMContentLoaded', async function() {
  ...
  function inicializarSemestres() {
    ...
    tarjetasExistentes.forEach(tarjeta => {
      ...
      // En la vista de Coordinador no se añaden botones de editar/eliminar
  ...
  if (btnCrearSemestre) {
    btnCrearSemestre.addEventListener('click', function() {
      ...
      if (formSemestre && typeof formSemestre.reset === 'function') formSemestre.reset(); // Limpiar formulario
    });
  }
  ...
  // Vista del Coordinador: deshabilitar la creación/edición desde esta página (si existe el formulario)
  if (formSemestre) {
  ...
  function agregarTarjetaSemestre(semestre, insertAtStart = true) {
    ...
    // Contenido de la tarjeta (vista de Coordinador: solo lectura, sin botones)
    tarjeta.innerHTML = `
  ...
  // Función para abrir el panel de la Unidad Demetrio (vista coordinador)
  function abrirPanelAdministrador(semestre) {
    ...
    // Indicar la unidad académica exacta que usan otras páginas para enrutar
    sessionStorage.setItem('unidad_academica', 'Unidad Demetrio Vallejo en el Espinal');

    // Redirigir al panel específico de Demetrio Vallejo
    window.location.href = 'U_Demetrio.html';
  }
